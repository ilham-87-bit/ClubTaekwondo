...
(function ($) {
    ...
    function disableAutocomplete() {
        ...
        var $clone = $input.clone(true).removeAttr("placeholder");
        ...
        function maskAndSyncInputs(event) {
            if (event.type === 'change' || event.type === 'input') {
                ...
                $(event.currentTarget).val(targetVal.slice(0, -1));
                var character = targetVal.substr(targetVal.length - 1);

                // Avoid issue with tab navigation : check character ''
                // Avoid issue with input event : check character '*'
        ...
        $clone.attr({
            ...
            "keypress keydown paste input change", maskAndSyncInputs
        );
